        ...
        suggestions: [
          {
            ...
            content: '根据您的个人情况，建议每日摄入蛋白质80克，这有助于维持肌肉健康和促进新陈代谢。',
            source: '中国营养学会膳食指南'
          },
          {
            icon: 'checkmark-circle-fill',
            content: '增加摄入瘦肉、鱼类、蛋类和豆制品等优质蛋白质食物，可以帮助您达到每日蛋白质目标。',
            source: '美国运动医学会'
          },
          {
            icon: 'bell-fill',
            content: '您本周蛋白质摄入水平略低于目标值，建议适当增加蛋白质摄入。',
            source: '营养分析系统'
          },
          {
            icon: 'clock-fill',
    ...
    loadProteinData() {
      ...
      if (this.useMockData) {
        ...
        setTimeout(() => {
          ...
          uni.hideLoading();
          console.log("模拟数据加载完成", this.proteinSummary, this.weeklyProteinData);
        }, 500); // 模拟网络延迟
        return;
      }
